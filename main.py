...
def analysis():
   ...
   ##second graph
    ...
    if (prediction[0]==0):
        ...
        report.config(text=f"Report:{0}",fg="#8dc53f")
        report1.config(text=f"{name},you do not have a heart disease")
        
    else:
        print("The prson has heart disease")
...
def Info(): 
    ...
    Label(icon_window,text="ca - number of major vessels (0-3) coloured by fluoroscopy",font="arial 11").place(x=20,y=430)
...
###it is for closing window
def logout() :
    root.destroy()
    
####Clear (with the healp of clear we can clear more entry field at once)
def Clear(): 
    Name.get('')
    DOB.get('')
    trestbps.get('')
